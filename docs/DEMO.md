...
| **2:30–3:30** | **Looker Priority Queue** | Display Looker Studio report showing Caregiver Priority Queue ranked by `risk_score DESC` and CPU vs GPU acceleration speedup benchmarks. | Looker Studio live dashboard with SHA-256 Checksum match (`TRUE`). |
| **3:30–4:30** | **Gemini AI Explanation** | Patient receives Risk Alert on mobile and taps "Explain with Gemini AI". Gemini synthesizes Thai advice without changing score or prescribing. | Mobile App Gemini AI Modal response. |
...
- [x] Looker Studio Priority Queue live
- [x] Gemini AI explanation active via Cloud Functions
