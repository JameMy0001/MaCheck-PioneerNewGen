        ...
        [
            ...
            make_table(
                ...
                [
                    ...
                    ["สถานะเอกสาร", "Production design v1.1 - แก้ไขช่องว่างและเพิ่มสกีมา migration เรียบร้อยแล้ว"],
                ],
                [40, 134],
            ),
            Spacer(1, 25 * mm),
...
def section_current_state(story: list) -> None:
    story.extend(
        [
            ...
            Spacer(1, 4 * mm),
            p("การแก้ไขช่องว่างระบบเพื่อทำ Agent จริง", H2),
            make_table(
                ["ช่องว่างเดิม", "ความคืบหน้าการแก้ไข (Resolved Status)", "สกีมาฐานข้อมูลที่รองรับการทำ Agent จริง"],
                [
                    ["โรคและแพ้ยาเป็น array/JSON รวม", "[แก้ไขแล้ว] สร้างตารางแยกเฉพาะและเก็บประวัติการทบทวน", "patient_conditions & patient_allergies (มี effective dates & verification)"],
                    ["ไม่มีน้ำหนัก/ข้อมูลร่างกายแบบ time series", "[แก้ไขแล้ว] สร้างตารางจัดเก็บค่าวัดร่างกายแบบมี timestamp", "body_metrics (พร้อม unit, source และ quality_flag)"],
                    ["dose_events เป็น boolean", "[แก้ไขแล้ว] ปรับเป็น structured status enum และระบุเหตุผล", "dose_events_v2 (สถานะ taken/late/missed/skipped และ reason)"],
                    ["ตารางกินยาอยู่ใน JSON", "[แก้ไขแล้ว] แผ่ข้อมูลตารางยาเป็นแถวข้อมูลเดี่ยว ป้องกัน JSON หลอน", "medication_schedules ( normalized fields + schedule_version)"],
                    ["ไม่มี snapshot/run lineage", "[แก้ไขแล้ว] สร้างตารางเก็บ snapshots และ logs ละเอียดระดับ run & step", "patient_snapshots, agent_runs, agent_tool_calls, agent_summaries"],
                    ["ไม่มี clinical protocol ที่ทบทวนแล้ว", "[แก้ไขแล้ว] ทะเบียนกฎการรักษา Deterministic ที่ควบคุมเวอร์ชันได้", "clinical_rules (rule registry ที่ publish โดยผู้ทบทวนแพทย์/เภสัชกร)"],
                ],
                [45, 59, 70],
            ),
            Spacer(1, 4 * mm),
